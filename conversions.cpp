...
string Conversions::binary2decimal(const std::string& bin) { //uzycie funkcji string to int do konwersji liczby binarnej na dziesietna
    int dec = std::stoi(bin, nullptr, 2);
    string str = std::to_string(dec);
    return str;
}

string Conversions::oct2decimal(const std::string& oct) {
    int dec = std::stoi(oct, nullptr, 8);  //uzycie funkcji string to int do konwersji liczby osemkowej na dziesietna
    string str = std::to_string(dec);
    return str;
}

string Conversions::hex2decimal(const std::string& hex) {
...
string Conversions::decimal2binary(const std::string& dec) {
...
string Conversions::decimal2oct(const std::string& dec) {     //inspiracja kodem programiz, kod zosta³ rozszerzony i zmodyfikowany do u¿ycia w moim programie
    if (dec == "error") {
        std::string error = "error, possibly wrong number input";
        return error;
    }
...
string Conversions::decimal2hex(const std::string& dec) {   //funkcja zainspirowana kodem ze strony geeksforgeeks, jednak zmodyfikowana do u¿ycia w moim programie
    if (dec == "error") {
        std::string error = "error, possibly wrong number input";
        return error;
    }
